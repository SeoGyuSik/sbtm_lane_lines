    ...
    try:
        ...
        if (len(left_line.recent_fit) == 0) | (len(right_line.recent_fit) == 0):
            ...
            if (dif_mid > dif_base * 0.8) & (dif_mid < dif_top * 1.2) & (dif_mid > 500) & (dif_mid < 900):
                ...
                s1 = 0
                s2 = 0
                for fit in left_line.recent_fit:
                    s0 += fit[0]
                    s1 += fit[1]
                    s2 += fit[2]
                m0 = s0 / len(left_line.recent_fit)
                m1 = s1 / len(left_line.recent_fit)
                m2 = s2 / len(left_line.recent_fit)
                left_line.best_fit = np.array([m0, m1, m2], dtype='float')

                s0_r = 0
                s1_r = 0
                s2_r = 0
                for fit in right_line.recent_fit:
                    s0 += fit[0]
                    s1 += fit[1]
                    s2 += fit[2]
                m0_r = s0_r / len(right_line.recent_fit)
                m1_r = s1_r / len(right_line.recent_fit)
                m2_r = s2_r / len(right_line.recent_fit)
                right_line.best_fit = np.array([m0_r, m1_r, m2_r], dtype='float')
                ...
                result = cv2.addWeighted(undist, 1, newwarp, 0.3, 0)
                result = show_txt(result)
                cv2.imwrite('te3.jpg', result)
            else:
                cv2.putText(image, "Can not found two parallel lines yet",
                            (30, 30), cv2.FONT_HERSHEY_DUPLEX, 1, (255, 255, 255), 1)
                cv2.imwrite('te3.jpg', image)
                # return image
